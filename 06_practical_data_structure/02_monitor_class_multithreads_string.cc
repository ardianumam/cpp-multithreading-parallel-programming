...
int main()
{
	...
	// Launch 5 async, each in new threads. Each receives lambda function with
    // two operations: string concat and printing. Each async returns future, 
    // and is appended into a a vector
	for (int i = 0; i < 5; ++i) { 
    ...
    // We iterate each future element in the vector. The order is guaranteed to be in the same order
    // of the vector element order, becaue future.wait() blocks the subsequent instructions 
	for (auto& fut: futs)
		fut.wait();
